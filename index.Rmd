---
title: 'data.table, glmnet, xgboost w/ caret - machine learning for predicting housing prices'
output:
  html_document:
...
Exploratory analysis and machine learning modal for predicting housing prices in competition ["House Prices: Advanced Regression Techniques"](https://www.kaggle.com/c/house-prices-advanced-regression-techniques). 
...
A description of the data is available on [Kaggle](https://www.kaggle.com/c/house-prices-advanced-regression-techniques/data). We've grouped the variables (data items) into 4 groups: square footage, counts, values, and factors. These groups will be used for engineering on the complete group rather than specifying one transformation at a time.
...
**NOTE**: The "None" values don't actually get added correctly here ... which is confusing. They get added in the [Missing Values](#Missing-values) section. 
...
```{r echo=FALSE}
descStats <- psych::describe(full.dt[, c(variablesSquareFootage,variablesValues), with = FALSE]) 
## Interactive HTML table
datatable(
  round(descStats,2), rownames = T,
  caption = "Descriptive statistics",
  options = list(pageLength = nrow(descStats))
)
```
...
```{r echo=FALSE}
tic()
zeroVarianceVariables.df <- nearZeroVar(
  train.dt, names = T, saveMetrics = T,
  foreach = T, allowParallel = T
)
toc()

datatable(
  round(subset(zeroVarianceVariables.df, nzv == TRUE, select = c("freqRatio","percentUnique")),2),
  rownames = T, caption = "Variables with (near) zero variance",
  options = list(pageLength = nrow(zeroVarianceVariables.df))
)
```
...
```{r echo=FALSE}
train.dt %>% ggplot(aes(GrLivArea, SalePrice)) +
...
```{r echo=FALSE}
train.dt %>% ggplot(aes(GrLivArea,LotArea)) +
  geom_point(shape=1) + ## Use hollow circles
  geom_smooth(
    method=lm, ## Add linear regression line
    se=FALSE
...
```{r echo=FALSE}
train.dt %>% ggplot(aes(as.factor(Neighborhood),SalePrice)) +
...
```{r echo=FALSE}
train.dt %>% ggplot(aes(OverallQual,SalePrice)) +
  geom_point(shape=1) +      ## Use hollow circles
  geom_smooth(
    method=lm, ## Add linear regression line 
    se=FALSE ## Don't add shaded confidence region
...
# PREVIOUSLY, NUMERIC INTEGERS were the only variables used
# THIS MEANS, we need to add our Date factors `variablesTime`
variablesNumeric <- colnames(full.dt)[sapply(full.dt, is.numeric)] ## both numeric integers
# corrData <- full.dt[dataPartition == "train", ..variablesNumeric]
corrData <- full.dt[dataPartition == "train", c(variablesNumeric, variablesTime), with=FALSE]
corrData[, (variablesTime) := lapply(.SD, as.numeric), .SDcols = variablesTime]
corMatrix <- cor(corrData)

## plot correlation
corrplot(
  corMatrix, title = "", type = "lower", order = "hclust",
  hclust.method = "centroid", tl.cex = 0.8, tl.col = "black", tl.srt = 45
)
```

## select highly correlated values

**NOTE:** The original analysis selects "highly correlated variables" that end up getting removed further down the line. Thus, we may be able to replace these variables with those they are highly correlated with. TBD, not 100% sure on the logic here. 

```{r}
## table with highly correlated values
corTable <- setDT(reshape2::melt(corMatrix))[order(-value)][value!=1]
...
This section heavily utilizes the `{caret}` package used for **C**lassification **A**nd **RE**gression **T**raining. Resource:
...
- https://htmlpreview.github.io/?https://github.com/zachmayer/caretEnsemble/blob/master/doc/caretEnsemble-intro.html
